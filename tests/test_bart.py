...
def test_bart_on_tables_vi(response):
    X = np.random.normal(0, 1, size=(250, 3))
    Y = np.random.normal(0, 1, size=250)
    X[:, 0] = np.random.normal(Y, 0.1)

    with pm.Model() as model:
        mu = pmb.BARTOnTables("mu", X, Y, m=10, response=response)
        sigma = pm.HalfNormal("sigma", 1)
        pm.Normal("y", mu, sigma, observed=Y)
        idata = pm.sample(tune=200, draws=200, random_seed=3415)
        vi_vals = idata["sample_stats"]["variable_inclusion"].values.ravel()
...
def test_bart_on_tables_missing_data(response):
    X = np.random.normal(0, 1, size=(50, 2))
    Y = np.random.normal(0, 1, size=50)
    X[10:20, 0] = np.nan
...
def test_bart_on_tables_shared_variable(response):
    X = np.random.normal(0, 1, size=(50, 2))
    Y = np.random.normal(0, 1, size=50)

    with pm.Model() as model:
        data_X = pm.Data("data_X", X)
        mu = pmb.BARTOnTables("mu", data_X, Y, m=2, response=response)
        sigma = pm.HalfNormal("sigma", 1)
        y = pm.Normal("y", mu, sigma, observed=Y, shape=mu.shape)
        idata = pm.sample(tune=100, draws=100, chains=2, random_seed=3415)
        ppc = pm.sample_posterior_predictive(idata)
...
def test_bart_on_tables_shape(response):
    X = np.random.normal(0, 1, size=(250, 3))
    Y = np.random.normal(0, 1, size=250)

    with pm.Model() as model:
        w = pmb.BARTOnTables("w", X, Y, m=2, response=response, shape=(2, 250))
        y = pm.Normal("y", w[0], pm.math.abs(w[1]), observed=Y)
        idata = pm.sample(tune=50, draws=10, random_seed=3415)
...
def test_bart_on_tables_moment(size, expected):
    X = np.zeros((50, 2))
    Y = np.zeros(50)
    with pm.Model() as model:
        pmb.BARTOnTables("x", X=X, Y=Y, size=size)
    assert_moment_is_expected(model, expected)
...
def test_bart_on_tables_categorical_model(separate_trees, split_rule):
    Y = np.array([0, 0, 0, 1, 1, 1, 2, 2, 2])
    X = np.concatenate([Y[:, None], np.random.randint(0, 6, size=(9, 4))], axis=1)
...
def test_multiple_bart_on_tables_variables():
    """Test that multiple BARTOnTables variables can coexist in a single model."""
    X1 = np.random.normal(0, 1, size=(50, 2))
    X2 = np.random.normal(0, 1, size=(50, 3))
    Y = np.random.normal(0, 1, size=50)
    ...
    with pm.Model() as model:
        # Two separate BARTOnTables variables with different covariates
        mu1 = pmb.BARTOnTables("mu1", X1, Y1, m=5)
        mu2 = pmb.BARTOnTables("mu2", X2, Y2, m=5)

        # Combined model
        sigma = pm.HalfNormal("sigma", 1)
        pm.Normal("y", mu1 + mu2, sigma, observed=Y)

        # Sample with automatic assignment of BARTOnTables samplers
        idata = pm.sample(tune=50, draws=50, chains=1, random_seed=3415)

        # Verify both BARTOnTables variables have their own tree collections
        assert hasattr(mu1.owner.op, "all_trees")
        assert hasattr(mu2.owner.op, "all_trees")
...
def test_multiple_bart_variables_manual_step():
    """Test that multiple BARTOnTables variables work with manually assigned PGBARTOnTables samplers."""
    X1 = np.random.normal(0, 1, size=(30, 2))
    X2 = np.random.normal(0, 1, size=(30, 2))
    Y = np.random.normal(0, 1, size=30)
    ...
    with pm.Model() as model:
        # Two separate BARTOnTables variables
        mu1 = pmb.BARTOnTables("mu1", X1, Y1, m=3)
        mu2 = pmb.BARTOnTables("mu2", X2, Y2, m=3)

        # Non-BARTOnTables variable
        sigma = pm.HalfNormal("sigma", 1)
        y = pm.Normal("y", mu1 + mu2, sigma, observed=Y)

        # Manually create PGBARTOnTables samplers for each BARTOnTables variable
        step1 = pmb.PGBARTOnTables([mu1], num_particles=5)
        step2 = pmb.PGBARTOnTables([mu2], num_particles=5)
